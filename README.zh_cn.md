...
由于该项目并不是使用源生的 coroutine server 来启动，因此，当你使用上述参数启动支持协程服务的时候，请求响应的字节流写入依然在主进程，但是所有其他的工作，包括你定义的控制器方法（无论使用 `async def` 与否），均运行在另一个线程当中。

*注意：Websocket 的控制类请不要使用`async def`。*

*注意：由于底层的server实现并不是使用协程，因此，所有的读写操作都是阻塞的，所以启用该模式时，请注意不要长期占用写出IO。*
